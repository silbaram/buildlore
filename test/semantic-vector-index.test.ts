...
function corpus(
  ...
  schemaVersion: ApprovedWikiRetrievalCorpusV1['schemaVersion'] =
...
describe('project-scoped flat-file semantic index', () => {
  it('indexes semantic text while preserving raw evidence and canonical locators', async () => {
    const body = `This page is generated by BuildLore.
source_digest: sha256:${'a'.repeat(64)}
Current retrieval keeps only meaning. [^citation-semantic-clean]`;
    const semanticText = 'Current retrieval keeps only meaning.';
    const base = page('semantic-fixture/clean', body, 'a');
    const baseSection = base.sections[0];
    if (baseSection === undefined) throw new Error('Fixture is incomplete.');
    const meaningSignal: ApprovedWikiMeaningSignalV1 = Object.freeze({
      authority: 'canonical',
      ...
      origin: 'manifest',
      revisionOrdinal: 25,
      sourceId: 'source-semantic-fixture-clean',
      topicGroup: 'retrieval',
    });
    const enrichedPage = Object.freeze({
      ...base,
      meaningSignals: Object.freeze([meaningSignal]),
      sections: Object.freeze([Object.freeze({
        ...baseSection,
        exclusionSummary: Object.freeze({
          ...
          excludedUtf8Bytes: Buffer.byteLength(body, 'utf8') -
            Buffer.byteLength(semanticText, 'utf8'),
          policyDigest: SEMANTIC_CONTENT_POLICY_DIGEST,
          policyVersion: SEMANTIC_CONTENT_POLICY_VERSION,
        }),
        meaningSignals: Object.freeze([meaningSignal]),
        semanticContentDigest: digestHierarchyValue(semanticText),
        semanticText,
      })]),
    });
    const value = corpus(
    ...
    const result = await chunkApprovedWikiCorpus(value, PROJECT_ID, (texts) =>
      Promise.resolve(texts.map((text) => Array.from(text).length)));

    expect(enrichedPage.sections[0]?.body).toBe(body);
    expect(result.chunks.map((chunk) => chunk.text)).toEqual([semanticText]);
    expect(result.chunks[0]?.chunk.citationLocators).toEqual([{
      citationId: 'citation-semantic-clean',
      sourceId: 'source-semantic-fixture-clean',
    }]);
    expect(result.chunks[0]?.chunk.meaningSignals).toEqual([meaningSignal]);
    expect(JSON.stringify(result.chunks)).not.toContain('source_digest');
    expect(JSON.stringify(result.chunks)).not.toContain('generated by BuildLore');
    expect(JSON.stringify(result.chunks)).not.toContain('[^citation-');
  });

  it('keeps semantic paragraph citations scoped to the claim that supplied them', async () => {
    const body = `Alpha claim explains the current architecture. [^citation-alpha]

Beta claim records the verification result. [^citation-beta]`;
    const semantic = projectApprovedWikiSemanticText(body, 'Fallback');
    const base = page('semantic-fixture/scoped-citations', body, 'c');
    const section = base.sections[0];
    if (section === undefined) throw new Error('Fixture is incomplete.');
    const meanings: readonly ApprovedWikiMeaningSignalV1[] = Object.freeze([
      Object.freeze({
        authority: 'canonical',
      ...
      Object.freeze({
        ...
        topicGroup: 'verification',
      }),
    ]);
    const value = corpus([Object.freeze({
      ...base,
      meaningSignals: meanings,
      sections: Object.freeze([Object.freeze({
        ...section,
        citationLocators: Object.freeze([
          Object.freeze({ citationId: 'citation-alpha', sourceId: 'source-alpha' }),
          Object.freeze({ citationId: 'citation-beta', sourceId: 'source-beta' }),
        ]),
    ...
    })], 'd', 'buildlore.approved-wiki-retrieval-corpus.v2');

    const result = await chunkApprovedWikiCorpus(value, PROJECT_ID, (texts) =>
      Promise.resolve(texts.map((text) => Math.max(1, Array.from(text).length))));

    expect(result.chunks.map(({ chunk }) => chunk.citationLocators)).toEqual([
      [{ citationId: 'citation-alpha', sourceId: 'source-alpha' }],
      [{ citationId: 'citation-beta', sourceId: 'source-beta' }],
    ]);
  });
  ...
  it('[HSW-A-12] preserves Markdown structures in stable path-free retrieval chunks', async () => {
    ...
    expect(first.chunks.map(({ chunk }) => chunk.citationLocators.length)).toEqual([1, 1, 1, 1, 1]);
...
\`\`\``;
  ...
  it('fills semantic top-k with distinct sections when one section owns the highest chunks',
    async () => {
      const knowledge = await knowledgeFixture();
      const value = corpus([
        page('semantic-fixture/guide', STRUCTURED_BODY, 'a'),
        page(
          'semantic-fixture/secondary',
          'Secondary semantic evidence. [^citation-semantic-fixture-secondary]',
          'b',
        ),
      ], 'c');
      const service = createFlatFileVectorIndex(knowledge);
      await service.buildFull(buildInput(value, fakeProvider([])));
      const query = vectorFor(splitMarkdownStructures(STRUCTURED_BODY)[0]?.text ?? 'missing');

      const result = await service.searchExactDistinctSections(PROJECT_ID, query, 2);

      expect(result.hits).toHaveLength(2);
      expect(new Set(result.hits.map((hit) => `${hit.pageId}:${hit.sectionId}`)).size).toBe(2);
    });
