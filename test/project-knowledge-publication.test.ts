...
import { createApprovedWikiProjectionStore, prepareApprovedWikiPublication, verifyApprovedWikiAuthority, type ApprovedWikiAuthorityV1 } from '../src/retrieval/approved-corpus-store.js';
...
describe('project knowledge publication recovery', () => {
  it('preserves unowned files on first activation, including a collision after the initial check', async () => {
    ...
    const authority = record(JSON.parse(await readFile(join(fixture.hubRoot, inputPath), 'utf8')) as unknown).authority as ApprovedWikiAuthorityV1;
  ...
  it('archives legacy authority once and restores all old bytes for each pre-commit failure', async () => {
    ...
    const authority = record(JSON.parse(await readFile(join(fixture.hubRoot, path), 'utf8')) as unknown).authority as ApprovedWikiAuthorityV1;
    verifyApprovedWikiAuthority(authority, projectId);
    const namespace = join(fixture.knowledgeRoot, 'projects', projectId, 'wiki/buildlore-hierarchy');
