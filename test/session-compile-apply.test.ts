...
describe('session compile proposal validation', () => {
  ...
  it('keeps structural page identity out of generic entropy scanning while scanning content fields', async () => {
    ...
    for (const value of [
      ...
      await expect(validateSessionProposalBatch(snapshot, [value], knowledgeRoot, 'alpha')).resolves.toBeDefined();
    }
  });

  it('continues scanning citation evidence after excluding its validated generated source identity', async () => {
    const unsafeQuote = ['gh', 'p_', '1234567890'.repeat(3), '123456'].join('');
